...
4. "예약하기" 버튼을 누르면 `api/book-slot.js`가 아래 단계를 순서대로 처리해요:
   ...
   2. **Jira 이슈 생성** — `OPER` 프로젝트의 "채용"(OPER-30) 하위에 면접 미팅 이슈를 자동으로 만들고, 그 이슈 키도 슬롯에 저장해요 (나중에 취소될 때 이 이슈를 지우기 위함).
...
- **취소**를 누르면: 그 슬롯이 다시 예약 가능한 상태로 열리고, 첨부했던 이력서/포트폴리오 파일은
  스토리지에서 삭제되고, 연결된 Jira 이슈도 자동으로 삭제되고, 채용 담당자에게도 취소 알림
  메일이 가요. 한 번 쓴 링크는 그 즉시 무효화돼서 재사용할 수 없어요.
...
| `jira_issue_key` | 이 예약으로 생성된 Jira 이슈 키. 취소 시 그 이슈를 지우기 위해 저장해둠 |
