...
C·D·E·F 중 하나가 실패해도(예: 메일 서버 일시 오류) **예약 자체(B)는 이미 끝난 뒤라 후보자에게는
...
4. "예약하기" 버튼을 누르면 `api/book-slot.js`가 아래 단계를 순서대로 처리해요:
   1. **예약 확정** — Supabase `interview_slots` 테이블에서 그 슬롯을 "예약됨"으로 바꾸고, 취소/변경용 개인 링크에 쓸 무작위 토큰(`cancel_token`)도 이때 같이 저장해요. (일반 키가 아니라 서버 전용 키로 처리하는 이유는 아래 "왜 service_role 키를 쓰나요?" 참고)
   2. **Jira 이슈 생성** — `OPER` 프로젝트의 "채용"(OPER-30) 하위에 면접 미팅 이슈를 자동으로 만들고, 그 이슈 키도 슬롯에 저장해요 (나중에 취소될 때 이 이슈에 코멘트를 남기기 위함).
   3. **AI 이력서 분석** — Claude API(`claude-sonnet-5`)가 첨부된 이력서/포트폴리오 PDF를 읽고, 채용 담당자가 면접 전에 1분 안에 훑어볼 수 있는 요약(강점, 확인이 필요한 점, 종합 인상)을 만들어요.
   4. **채용 담당자 알림 메일** — Resend를 통해 `newlearnsoft@gmail.com`으로 예약 정보 + Jira 링크 + 위 AI 분석 결과가 담긴 메일을 보내요.
   5. **후보자 확인 메일** — 후보자 본인에게도 예약 확인 메일을 보내는데, 여기에 취소/변경을 할 수 있는 개인 링크(`manage.html?token=...`)가 담겨 있어요.
...
### 4️⃣ 후보자가 예약을 취소·변경할 때 (`manage.html`, `api/manage-booking.js`)

예약 확인 메일에 있는 개인 링크로 들어가면, 본인 예약 정보를 확인하고 취소할 수 있어요.

- 링크는 `manage.html?token=무작위값` 형태예요. 이 토큰은 예약 확정 시 한 번만 발급되는,
  추측이 사실상 불가능한 값이라(6자리 숫자 코드 같은 걸 쓰지 않는 이유예요) 다른 사람이 URL을
  안다고 해도 남의 예약을 건드릴 수 없어요.
- **취소**를 누르면: 그 슬롯이 다시 예약 가능한 상태로 열리고, 첨부했던 이력서/포트폴리오 파일은
  스토리지에서 삭제되고, 연결된 Jira 이슈에는 "후보자가 취소했다"는 코멘트가 자동으로 남고,
  채용 담당자에게도 취소 알림 메일이 가요. 한 번 쓴 링크는 그 즉시 무효화돼서 재사용할 수 없어요.
- **변경**은 별도 화면 없이, 취소 후 같은 담당자가 미리 선택된 상태로 예약 페이지(`index.html`)로
  이동시켜서 새 시간을 고르게 하는 방식으로 처리해요 — 화면 두 개를 따로 만드는 것보다 훨씬 단순해요.
...
manage.html          후보자 예약 확인/취소 페이지 (이메일 링크로 진입)
style.css            공통 스타일
...
### `interview_slots` 테이블 주요 컬럼

기본 컬럼(`slot_date`, `start_time`, `end_time`, `is_booked`, `interviewer`, `candidate_*`, `resume_url`,
`portfolio_url`, `desired_salary`, `booked_at`) 외에 취소 기능을 위해 추가된 컬럼:

| 컬럼 | 역할 |
|---|---|
| `cancel_token` | 후보자 취소/확인 링크에 들어가는 무작위 값. 예약 확정 시 생성되고, 취소되면 `null`로 비워져서 링크가 재사용되지 않음 |
| `jira_issue_key` | 이 예약으로 생성된 Jira 이슈 키. 취소 시 그 이슈에 코멘트를 남기기 위해 저장해둠 |
